...
public class DefaultAiPromptRegistry {
    ...
    private static final String WORD_QA_SYSTEM_PROMPT = "你是 LexiFlow 的 AI 英语学习助手。请只输出合法 JSON，不要输出 JSON 外的 Markdown、解释性前后缀或代码块。answer 字段的字符串内容可以使用 Markdown 语法。内容面向备考大学生，中文为主，简洁、准确、适合背单词。";
    private static final String WORD_QA_INSTRUCTION_PROMPT = String.join("\n",
            ...
            "输出 JSON 字段含义：answer 是直接回答用户问题的正文，可以少量使用 Markdown；keyPoints 是 2-4 条短要点，用普通文本概括最值得记的规则、搭配或辨析；relatedWords 是与当前单词确实相关、能帮助辨析或记忆的单词、短语、缩写或表达，每项建议写成“表达 (中文含义/关系)”；followUps 是 2-3 个适合继续追问的中文问题。",
            "回答必须直接回应 question，不能编造未给出的固定知识；如果问题超出单词学习范围，请简短说明并拉回该单词。不要在 JSON 对象外输出 Markdown。answer 可以适当使用加粗、反引号、列表和换行，但不要使用 Markdown 引用块，不要给中文翻译添加破折号、引号等额外前缀，不要过度加粗普通中文释义；keyPoints、relatedWords、followUps 不要使用 Markdown 标记。",
    ...
    private static final String CLOZE_QUIZ_INSTRUCTION_PROMPT = "严格规则：1. passage 必须包含 blankWords 中每个 word 的原词或常见词形变化，且每个 word 对应的 usedForm 在 passage 中只出现一次；2. explanations 必须为每个 blankWords 中的 word 返回一项，word 写原词，usedForm 写 passage 中实际出现的完整词形，例如 word 为 resemble 且文中使用 resembles，则 usedForm 写 resembles；3. 只能依据后端提供的主词性、主释义、全部词性和全部释义选择最合适的义项，不要自造未给出的词义；4. passage 不得出现中文释义、英文释义、词性解释、because it relates to 或类似泄题模板；5. 不要输出 ___1___ 这类占位符，后端会按实际出现位置自动挖空并生成正确答案；6. backgroundWords 是软约束，尽量自然融入 passage，影响通顺时可以省略；7. 不要输出 candidateWords 或 blanks，候选词和答案由后端程序生成；8. passage 要自然连贯，控制在 200-250 个英文词，并且必须符合英语语法，不能有语法错误；9. passageZh 要翻译整篇短文；10. explanations 里的 usedPos 和 definitionZh 必须对应你在 passage 里真正采用的那个义项，reasonZh 要简短解释为什么这里选这个词，且必须是中文；11. 若某个词有多个义项，优先选择最符合上下文且最自然的那个。";
