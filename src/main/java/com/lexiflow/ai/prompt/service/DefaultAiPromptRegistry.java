...
public class DefaultAiPromptRegistry {
    ...
    private static final String CLOZE_QUIZ_INSTRUCTION_PROMPT = "严格规则：1. passage 必须逐字包含 blankWords 中每个 word，且每个 word 在 passage 中只出现一次；2. 只能依据后端提供的主词性、主释义、全部词性和全部释义选择最合适的义项，不要自造未给出的词义；3. passage 不得出现中文释义、英文释义、词性解释、because it relates to 或类似泄题模板；4. 不要输出 ___1___ 这类占位符，后端会按实际出现位置自动挖空并生成正确答案；5. backgroundWords 是软约束，尽量自然融入 passage，影响通顺时可以省略；6. 不要输出 candidateWords 或 blanks，候选词和答案由后端程序生成；7. passage 要自然连贯，控制在 200-250 个英文词，并且必须符合英语语法，不能有语法错误；8. passageZh 要翻译整篇短文；9. explanations 里的 usedPos 和 definitionZh 必须对应你在 passage 里真正采用的那个义项，reasonZh 要简短解释为什么这里选这个词，且必须是中文；10. 若某个词有多个义项，优先选择最符合上下文且最自然的那个。";
