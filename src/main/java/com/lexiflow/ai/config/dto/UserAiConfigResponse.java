...
public record UserAiConfigResponse(
    ...
    public static UserAiConfigResponse empty() {
        return new UserAiConfigResponse(false, null, null, null, false, null);
    }

    public static UserAiConfigResponse from(UserAiConfig config) {
        return new UserAiConfigResponse(true, config.getApiBaseUrl(), config.getModelName(), config.getTemperature(), config.getEnabled(), config.getLastVerifiedAt());
    }
}
