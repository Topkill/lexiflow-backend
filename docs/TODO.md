...
- 边界风险：如果第一次生成仍在后端执行中，用户刷新页面后再次点击生成，当前后端没有按 `dailyTaskId + sourceType + targetWordCount` 合并 in-flight 任务，可能重复触发一次 AI 完形填空生成，增加等待时间和 AI 调用成本。
...
7. 后端增加同一用户、同一学习组、同一生成来源的 in-flight 任务复用或幂等拦截：已有 `PENDING/RUNNING` 任务时直接返回该任务，避免刷新、多标签或多设备重复调用 AI。
