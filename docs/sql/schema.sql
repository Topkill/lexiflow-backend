...
CREATE TABLE IF NOT EXISTS `study_event` (
  ...
  `algorithm_version` VARCHAR(32) NULL COMMENT '算法版本号，历史事件为 NULL，新事件由应用写入',
  `ef_before` DECIMAL(3, 2) NULL COMMENT '变更前 EF',
  `interval_days_before` INT NULL COMMENT '变更前间隔(天)',
  `repetition_before` INT NULL COMMENT '变更前连续成功次数',
  `status_before` VARCHAR(20) NULL COMMENT '变更前掌握状态',
  `ef_after` DECIMAL(3, 2) NULL COMMENT '变更后 EF',
  `interval_days_after` INT NULL COMMENT '变更后间隔(天)',
  `repetition_after` INT NULL COMMENT '变更后连续成功次数',
  `status_after` VARCHAR(20) NULL COMMENT '变更后掌握状态',
  `next_review_date_after` DATE NULL COMMENT '计算出的下次复习日期',
  PRIMARY KEY (`id`),
